...
class Temai extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $n = 0;
            $arr = [];
            $shopObj = db('mall_shop')->field('temai_end_time')->find();
            foreach ($list as $k => $v){
                if ($v['active_type'] != 3) {
                    unset($list[$k]);
                    $n++;
                    ...
                    $list[$k]['end_time'] = $shopObj['temai_end_time'];
                    $arr[] = $list[$k];
                }
            }
    ...
    public function add()
    {
        if ($this->request->isPost())
        {
...

            {
                ...
                try
                {
                    ...
                    $shopObj = db('mall_shop')->field('temai_start_time,temai_end_time')->find();
                    db('mall_goods')->where("goods_id={$params['goods_id']}")->update(['active_id'=>$this->model->active_id,'promote_price'=>$params['active_price'],'promote_start_time'=>$shopObj['temai_start_time'],'promote_end_time'=>$shopObj['temai_end_time']]);
                    if ($result !== false)
                    {
                        $this->success();
                    }
        ...
        $shopObj = db('mall_shop')->field('temai_start_time,temai_end_time')->find();
        $row['temai_start_time'] = $shopObj['temai_start_time'];
        $row['temai_end_time'] = $shopObj['temai_end_time'];
    ...
    public function edit($ids = NULL)
    {
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
                $shopObj = db('mall_shop')->field('temai_start_time,temai_end_time')->find();
            $params['start_time'] = $shopObj['temai_start_time'];
            $params['end_time'] = $shopObj['temai_end_time'];
            {
                try
                {
                    //是否采用模型验证
                    ...
                    if ($result !== false)
                    {
                        db('mall_goods')->where("goods_id={$params['goods_id']}")->update(['promote_price'=>$params['active_price'],'promote_end_time'=>$params['end_time']]);
                        $this->success();
                    }
                    else
                    {
