...
class WeixinPay {
    ...
    protected $openid;
    protected $total_fee;
    protected $notify_url;//支付结果通知接口
    protected $out_trade_no;//商户订单号
    function __construct($appid, $openid, $mch_id, $key, $total_fee , $notify_url ,$out_trade_no) {
        $this -> appid = $appid;
        $this -> openid = $openid;
        $this -> mch_id = $mch_id;
        $this -> key = $key;
    ...
    private static function postXmlCurl($xml, $url, $second = 30) {
        ...
        if ($data) {
            ...
            $error = curl_errno($ch);
            curl_close($ch);
            throw new WxPayExcrption("curl出错，错误码:$error");
        }
    }
